{{-- <script src="{{ asset('admin/file-manager/file-manager.js') }}"></script>
    <script src="{{ asset('admin/compiled/js/summernote-costum.js') }}"></script> --}}
    {{-- <script>
        class FileManager {
            constructor(options) {
                            ...
                            const list = `
                                            <div class="col mt-2 mb-4" style="cursor: pointer;position:relative">
                                                ...
                                                <div class="checkbox-container">
                                                    <input type="checkbox" class="form-check-input checked-file" style="width:1.5em;height:1.5em" value="${item.file}">
                                                </div>
                                            </div>`;
                            views.forEach(view => view.insertAdjacentHTML('beforeend',
                                list));
                        });
                    }
                });
            ...
            setupEventListeners() {
                ...
                document.body.addEventListener('click', (event) => {
                    ...
                    if (event.target.classList.contains('dropdown-item') && event.target.classList.contains(
                            'filters')) {
                ...
                document.body.addEventListener('change', (event) => {
                    if (event.target.classList.contains('form-check-input') && event.target.classList.contains(
                            'checked-file')) {
                        let fileName = event.target.value;
                        console.log(this.settings.multipleSelect)
                        if (this.settings.multipleSelect) {
                            // Jika multipleselect adalah true, maka bisa memilih lebih dari satu file
                            if (event.target.checked) {
                                // Checkbox diceklis, tambahkan nilai ke array jika belum ada
                                if (!this.selectedFiles.includes(fileName)) {
                                    this.selectedFiles.push(fileName);
                                }
                            } else {
                                // Checkbox tidak diceklis, hapus nilai dari array jika ada
                            ...
                            // Jika multipleselect adalah false, hanya boleh dipilih satu file
                            if (event.target.checked) {
                                // Checkbox diceklis, hapus nilai dari array dan unchecked file sebelumnya
                                this.selectedFiles = [fileName];
                                document.querySelectorAll('.form-check-input.checked-file').forEach((
                                    checkbox) => {
                                    if (checkbox.value !== fileName) {
                                        checkbox.checked = false;
                                        let index = this.selectedFiles.indexOf(checkbox.value);
                                        if (index !== -1) {
                                            this.selectedFiles.splice(index, 1);
                                        }
                                    }
                                ...
                                // Checkbox tidak diceklis, hapus nilai dari array
                                let index = this.selectedFiles.indexOf(fileName);
                                if (index !== -1) {
                                    this.selectedFiles.splice(index, 1);
                                }
    ...
    {{-- <script>
        const fileManager = new FileManager({
            listFilesUrl: '{{ route('api.filemanager.index') }}', // Change to your actual API URL
            summernoteSelector: '.isi-berita',
            modalSelector: '#file-manager',
            endPointUploadFile: '{{ route('api.filemanager.store') }}',
            _tokenCsrf: '{{ csrf_token() }}',
            previewImageUpload: false
        });

        const summernoteCustom = new SummernoteCustom({
            elementEditor: '.isi-berita',
            heightEditor: '40vh'
        });
    </script> --}}